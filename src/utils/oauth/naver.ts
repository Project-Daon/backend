...
async function requestAccessToken(resValue: Record<string, string>): Promise<NaverLoginVO> {
    const data = {
    ...
    const response = await axios.post<NaverLoginVO>('https://nid.naver.com/oauth2.0/token', new URLSearchParams(data));
